#!/usr/bin/env python
u"""
read_ICESat2_ATL10.py (11/2021)
Read ICESat-2 ATL10 (Sea Ice Height) data files
...
def read_HDF5_ATL10(FILENAME, ATTRIBUTES=False, **kwargs):
    """
    Reads ICESat-2 ATL10 (Freeboard) data files
    ...
    for gtx in IS2_atl10_beams:
        ...
        for key,val in fileID[gtx]['freeboard_beam_segment'].items():
            if isinstance(val, h5py.Dataset):
                IS2_atl10_mds[gtx]['freeboard_beam_segment'][key] = val[:]
            elif isinstance(val, h5py.Group):
                for k,v in val.items():
                    IS2_atl10_mds[gtx]['freeboard_beam_segment'][key][k] = v[:]
        ...
        if ATTRIBUTES:
            ...
            for key,val in fileID[gtx]['freeboard_beam_segment'].items():
                IS2_atl10_attrs[gtx]['freeboard_beam_segment'][key] = {}
                for att_name,att_val in val.attrs.items():
                    IS2_atl10_attrs[gtx]['freeboard_beam_segment'][key][att_name] = att_val
                if isinstance(val, h5py.Group):
                    for k,v in val.items():
                        IS2_atl10_attrs[gtx]['freeboard_beam_segment'][key][k] = {}
                        for att_name,att_val in v.attrs.items():
                            IS2_atl10_attrs[gtx]['freeboard_beam_segment'][key][k][att_name] = att_val
    ...
    #-- freeboatrd ancillary information (processing flags and parameters)
    cal = 'freeboard_estimation'
...
def find_HDF5_ATL10_beams(FILENAME):
    """
    Find valid beam groups within ICESat-2 ATL10 data files
