...
def get_neighbor_dyn_range_postproc(row, data, var, tree, dist_threshold):
    point = (row['geometry'].x, row['geometry'].y)
    if np.sqrt(2) * row['drift_unc'] < dist_threshold:
        return 0.0

    neighbors_indices = tree.query_ball_point(point, np.sqrt(2)*row['drift_unc'], p=2.0)
