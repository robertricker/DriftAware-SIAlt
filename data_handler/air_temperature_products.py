...
class AirTemperatureProducts:
    ...
    def thermodyn_growth(self, thermo_model, hice, hsnow, x, y, direct, oce_heat_flux_product):
        ...
        F = oce_heat_flux_product # the ocean heat flux, is assumed to be constant #TODO, take it not constant ?
        dt = 86400 # daily

        if thermo_model == 'winter_2layers':
            if direct == 1:
                ...
                # Discriminant :
                D = b**2 - 4*a*c
                s1 = (-b - np.sqrt(D.where(D >= 0))) / (2 * a)
                #s2 = (-b+np.sqrt(D))/(2*a)
                deltaH = (Hf - s1) #already in the time direction
                Hf = s1.copy() #that is in fact Hice ... 
                if (D<0).values.any():
                    logger.info('No real solution to the polynom for all the values, some discriminant are negatives :%s' %(D<0).values.sum())
